...
{
	if(indice[0]=='9'){
		/*	Ti.App.Properties.removeProperty("HS_gestos8");
		Ti.App.info(Ti.App.Properties.getBool("HS_gestos9", true));*/
		
		win1.add(gifView);
		//Desbloquear siguiente nivel.
		Ti.App.Properties.removeProperty("HS_gestos8");
		Ti.App.Properties.setBool("HS_gestos8",true);
		setTimeout(function(){
		/*desbloquear sgte. nivel
		if(Ti.App.Properties.getBool("HS_gestos9") == 'true'){*/
			juego1=Alloy.createController('gestos_nivel_9');
		/*}else{
		...
		if(indice[0]=='7' && intentos<1){
			
			intentos=intentos+1;
		}
		else {
			if(indice[0]=='10' && intentos<1){
				
				intentos=intentos+1;
			}
			else{
				if(indice[0]=='12' && intentos<1){
					
					intentos=intentos+1;
				}
				else{
					
					juego7=Alloy.createController('gestos_nivel_8');
				}
			}
		}
	}
...
button2.addEventListener('click',function(e)
{
	if(indice[1]=='9'){
		...
		if(indice[1]=='7' && intentos<1){
			
			intentos=intentos+1;
		}
		else {
			if(indice[1]=='10' && intentos<1){
				
				intentos=intentos+1;
			}
			else{
				if(indice[1]=='12' && intentos<1){
...
button3.addEventListener('click',function(e)
{
	if(indice[2]=='9'){
		...
		if(indice[2]=='7' && intentos<1){
			
			intentos=intentos+1;
		}
		else {
			if(indice[2]=='10' && intentos<1){
				
				intentos=intentos+1;
			}
			else{
				if(indice[2]=='12' && intentos<1){
...
button4.addEventListener('click',function(e)
{
	if(indice[3]=='9'){
		...
		if(indice[3]=='7' && intentos<1){
			
			intentos=intentos+1;
		}
		else {
			if(indice[3]=='10' && intentos<1){
				
				intentos=intentos+1;
			}
			else{
				if(indice[3]=='12' && intentos<1){
